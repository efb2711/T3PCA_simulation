T3PCAsegmented <- function( X3D , Y , r1 , r2 , r3 , conv , OriginalAlfa , AlternativeLossF , nRuns , SegmentedType , StartSeed )
{
  # INPUT
  ...
  #    SegmentedType: type of segmentation
  #       1 = first Tucker, next constrained PCA (common component matrix should be the same)
  #       2 = first PCA, next constrained Tucker3 (common component matrix should be the same)
  ...
  {
    cat(" ",fill=TRUE)
    cat("SegmentedType should be 1 (T3 first) or 2 (PCA first)",fill=TRUE)
    cat(" ",fill=TRUE)
    checkinput=0
  }
  ...
  {
    cat(" ",fill=TRUE)
    cat("AlternativeLossF should be 0 (original) or 1 (alternative loss function)",fill=TRUE)
    cat(" ",fill=TRUE)
    checkinput=0
  }
  ...
  if (checkinput == 1)
  {
    ...
    cputime = system.time(
    {
      if( SegmentedType == 1 )
      {
        ...
        BestH = BestSol3D$H
        BestB2 = t( solve( t( BestA ) %*% BestA ) %*% t( BestA ) %*% Y ) #is in general not orthogonal !!
      ...
      else
      {
        ...
        BestSol3D = T3constrained( X , BestA , i , j , k , r1 , r2 , r3 , 0 , .0000001 , AnalysisSeeds[1] )
        FitValues[1] = BestSol3D$f
        nIterValues[1] = BestSol3D$iter
        for ( runtel in 1:nRuns )
        {
          tempSol3D = T3constrained( X , BestA , i , j , k , r1 , r2 , r3 , 1 , .0000001 , AnalysisSeeds[ runtel+1 ] )
          FitValues[ runtel+1 ] = tempSol3D$f
          nIterValues[ runtel+1 ] = tempSol3D$iter
          if( tempSol3D$f < BestSol3D$f )
          {
